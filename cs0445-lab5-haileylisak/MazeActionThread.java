...
public class MazeActionThread extends ActionThread
{
    ...
    public void executeApplication()
    {
        ...
        /*DONT FORGET THIS METHOD DONT FORGET THIS METHOD DONT FORGET THIS METHOD */
    ...
    private class MazeFrame
    {
        public int row;
        public int col;
        //represents one square of the maze
    ...
    {
        MazeFrame coordinates;
        boolean result = false;
        Stack<MazeFrame>MazeFrameStack = new Stack<MazeFrame>();
        MazeFrameStack.push(new MazeFrame(startRow, startCol));
        theMaze.scheduleSquare(startRow, startCol);
        while(result==false && MazeFrameStack.size()!=0)
        {
           coordinates = MazeFrameStack.pop();
           theMaze.visitSquare(coordinates.row, coordinates.col);
           animationPause();

           if(theMaze.isGoal(coordinates.row, coordinates.col))
           {
                result = true;
           }
           else
           {
                if(theMaze.isFree(coordinates.row+1, coordinates.col)==true)
                {
                    
                    theMaze.scheduleSquare(coordinates.row+1, coordinates.col);
                    MazeFrameStack.push(new MazeFrame(coordinates.row+1, coordinates.col));
                    animationPause();
                }
                if(theMaze.isFree(coordinates.row, coordinates.col+1)==true)
                {
                    
                    theMaze.scheduleSquare(coordinates.row, coordinates.col+1);
                    MazeFrameStack.push(new MazeFrame(coordinates.row, coordinates.col+1));
                    animationPause();
                }
                if(theMaze.isFree(coordinates.row, coordinates.col-1)==true)
                {
    ...
    //pop item off top of stack and check if it's the goal
            //mark item as visited
                //if it is the goal, then set result to true
                //if it is not the goal, check each direction is an available spot
                    //if it's an availble spot, then schedule a visit and push to stack

        /*
         * every time you pop off stack, mark as visited
         * every time you push onto stack, mark as scheduled
         * every time you call visitSquare or scheduleSquare, you MUST call animatePause() afterwards
         */
