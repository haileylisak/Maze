...
/*
 * check if start point is the goal
 * check up, if available then push to call stack 
 * check down, check right, check left
 *          if they are available spots, push frame to stack
 * 
 * pop off the left frame because that is the top of stack and check if thats the goal
 *      
 * 
 * 2D array of booleans
 * 1 means black square
 * 0 means white square (open spot)
 * 5 5 is the dimensions 5x5
 * 
 * this is the class we will call methods from we dont actually write in this class
 * 
 * dont worry about setGoal
 * 
 * after that use the next 4 methods
 * 
 * visitSquare: makes the sqaure light blue (shows that we visited that square already)
 * scheduleSquare: marks it scheduled, or dark blue circle
 * isGoal: checks if its the goal
 * isFree: checks if the square is available to schedule a visit(basically is it open or white square)
 */